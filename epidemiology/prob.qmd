...
The binomial model is probably the most commonly encountered probability distribution in epidemiology, and most epidemiologists should be at least moderately familiar with it. The probability that an event occurs $k$ times in $n$ trials is described by the formula $\binom{n}{k} p^k q^{n-k}$ where $p$ is the probability of the event occurring in a single trial, $q$ is the probability of the event not occurring ($1-p$) and $\binom{n}{k}$ is the binomial expansion "n choose k" the formula for which is $latex \frac{n!}{k!(n-k)!}$
...
-   y has form $latex y \sim B(n, F(\beta_0 + \beta_1x)$

-   where $latex F(z) = e^z / (1+e^z)$ ,i.e. logit, and
-   LD50 = $latex \frac{-\beta}{\beta_1}$ i.e. point at which argument of the  distribution function is zero
...
The Poisson, gamma and exponential distributions are related.  Counts on a timeline tend to be Poisson distributed, $latex Pr[k]= e^{-\lambda} * \lambda^{k} /k! $.  The time periods between the counts, tend to be exponentially distributed, with a single parameter, $latex \lambda = rate = \mu = \sigma^{2}$.  The exponential distribution in turn is a instance of a *gamma* distribution.  

Gamma distributions are defined as the sum of *k* independent exponentially distributed random variables with two parameters:  a scale parameter,  $latex \theta$ , and a shape parameter, $latex \kappa$. The mean for a gamma distribution is $latex \mu=\theta \kappa$.  The shape parameter $latex \kappa$ is the number of sub-states in the outcome of interest.  In an exponential distribution there is a single transition state and you are counting every observation, so $latex \kappa =1$.  If you were interested in some process where you counted every other observation on a timeline, for example if there is some incubation period, you could model it with a  $latex \thicksim \Gamma(\lambda, 2)$

Because we do as epidemiologists spend a lot of time counting disease occurrences, you can get a lot of epidemiologic mileage from a Poisson distribution.  R makes working with Poisson distributed data fairly straightforward. Use *dpois()* to return the density function ($latex Pr[X=x] = \frac{x^{−\lambda} \lambda^{x}}{x!}$ , where $latex X =rv$, $x$ is the observed count, and $latex \lambda$ is the expected count) is. Use *ppois()* to return the CDG ($latex Pr[X \leq x] = \sum_{k=0}^ x \frac{k^{−\lambda} \lambda^{k}}{k!}$).
...
-   $latex \sigma_{r eval=FALSE, echo=TRUE} = \sqrt{x/py^2}$

-   $latex r_{L}; r_{U} = r \pm z × \sigma_{r eval=FALSE, echo=TRUE}$
...
- $H_{0}: \hat{r eval=FALSE, echo=TRUE} = r_{0}$
- $p = Pr[r \geq \hat{r eval=FALSE, echo=TRUE} | r = r_{0}]$
...
-   $\hat{r eval=FALSE, echo=TRUE} = 8/160 = 0.05,  r_{0} = 0.03$
-   $p = Pr[r \geq \hat{r eval=FALSE, echo=TRUE}=.05 | r_{0}=.03]$
...
$latex f(x) = \frac{1}{\sqrt{2\pi \sigma^2}} \left(\exp \frac{-\left(x-\mu\right)^2}{2\sigma^2}  \right)$
