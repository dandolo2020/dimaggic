...
[I. Grid Sampling](#grid-sampling)      
[1. The Multi-Parameter Normal Model](##the-multi-parameter-normal-model)     
[2. Random Sampling From a Standard Distribution](##random-sampling-from-a-standard-distribution)    
[3. Grid Sampling From a Non-Standard Distribution](##Grid-sampling-from-a-non-standard-distribution)      
[4. Grid Sampling for Airline Crash Example](##grid-sampling-for-airline-crash-example)   

[II. MCMC](#MCMC)         
[1. Introduction to Monte Carlo](##introduction-to-monte-carlo)     
[2. Introduction to Markov Chain Monte Carlo](##introduction-to-markov-chain-monte-carlo)     
[3. The Metropolis Algorithm](###the-metropolis-algorithm)            
[4. Gibbs Sampling](###gibbs-sampling)                
[5. Running MCMC in BUGS](##running-mcmc-in-bugs)              
...
In the [first part](http://www.columbia.edu/~cjd11/charles_dimaggio/DIRE/styled-4/styled-11/code-4/) of this discussion, we considered relatively simple one-parameter models and conjugate analysis.  Before we move on to the kinds of Markov Chain Monte Carlo methods in common use for more complex problems, we'll take some first steps toward realistic problems that require computational methods. The first such method is called grid sampling, which we present in the setting of both normal and Poisson models.  

## The Multi-Parameter Normal Model

 
In our [introduction](lhttp://www.columbia.edu/~cjd11/charles_dimaggio/DIRE/styled-4/styled-11/code-4/) to the single-parameter normal model, we assumed (somewhat unrealistically) that we didn't know the mean, but somehow did know the variance.  We now take the more real-world stance that both the mean and the variance are unknown.  So now, $\sigma^2$ doesn't drop out of the model specification, making the Bayesian calculations more complicated. You may begin to appreciate that even for this relatively simple model, the simple analytic approaches we've seen in the previous conjugate analyses become increasingly more difficult to apply, and we begin to see the need for computational approaches.[^Suspect]
